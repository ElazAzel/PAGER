...
export const COLLECTIONS = ["users", "pages", "publishedPages", "items", "contacts", "opportunities", "bookings", "orders", "subscriptions", "entitlements", "timeline", "integrations", "analytics", "assets", "webhooks", "notifications", "adminAudit", "creatorInvites"] as const satisfies ReadonlyArray<keyof DatabaseState>;
...
export function diffState(before: DatabaseState, after: DatabaseState): Change[] {
  ...
  for (const collection of COLLECTIONS) {
    ...
    for (const row of after[collection]) {
      ...
      if (collection === "adminAudit" && old && !isDeepStrictEqual(old, row)) throw new Error("Admin audit is append-only");
    ...
    for (const old of before[collection]) if (!seen.has(old.id)) {
      if (collection === "adminAudit") throw new Error("Admin audit is append-only");
      changes.push({ collection, operation: "delete", id: old.id, row: old });
    }
  }
  return changes;
}
export function assertStateShape(value: unknown): asserts value is DatabaseState {
  ...
  // Forward-compatible normalization for local demo snapshots made before pilot migrations.
  for (const key of ["adminAudit", "creatorInvites"]) if (!(key in value)) (value as Record<string, unknown>)[key] = [];
