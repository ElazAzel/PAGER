...
import { appearanceOf, blockAppearanceOf } from "../appearance";
import { sanitizeBlockData } from "./sanitize";
import { assertPageAvailable, getCapabilities, isPageAvailable } from "./capabilities";
...
export function canAccessBlock(page: Page, block: Block, userId: string | undefined, entitlements: Entitlement[], now = new Date()): boolean {
  ...
  return isPageAvailable(page) && (userId === page.ownerId || !block.hidden) && canReadBlockMaterial(page, block, userId, entitlements, now);
}
...
export function canReadBlockMaterial(page: Page, block: Block, userId: string | undefined, entitlements: Entitlement[], now = new Date()): boolean {
  ...
  if (!isPageAvailable(page)) return wholePage || thisBlock;
...
function projectPageContent(page: Page, userId: string | undefined, entitlements: Entitlement[], demo: boolean, includePurchasedHidden: boolean): PublicPage {
  ...
  return { id: page.id, slug: page.slug, title: page.title, description: pageAccess ? page.description : page.teaser, locale: page.locale, accent: page.accent, appearance: appearanceOf(page.appearance), paid: page.paid, teaser: page.teaser, pricing: { ...page.pricing }, publishedAt: page.publishedAt, updatedAt: page.updatedAt, revision: page.revision, owner, demo, locked: !pageAccess,
    blocks: page.blocks.filter(b => owner || (b.hidden ? includePurchasedHidden && canRead(page, b, userId, entitlements) : !b.archived || canRead(page, b, userId, entitlements))).map(b => {
      ...
      return { id: b.id, type: b.type, width: b.width, hidden: b.hidden, archived: b.archived, paid: b.paid, teaser: b.teaser, pricing: { ...b.pricing }, locked: !accessible, ...(b.appearance ? { appearance: blockAppearanceOf(b.appearance) } : {}), ...(accessible ? { data: sanitizeBlockData(b.data, b.type === "custom_code") } : {}) };
    }),
  };
}
...
export function projectPublicPage(state: DatabaseState, page: Page, userId?: string, demo = false, includePurchasedHidden = false): PublicPage {
  if (!includePurchasedHidden) assertPageAvailable(page);
