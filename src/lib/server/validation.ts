...
export const itemSchema = z.object({ id: idSchema, ownerId: idSchema, pageId: idSchema, title: z.string().min(1).max(200), description: z.string().max(20_000), kind: z.enum(["service", "digital", "physical", "ticket"]), price: moneySchema.positive(), currency: currencySchema, image: z.string().max(2048).optional(), category: short.optional(), stock: z.number().int().min(0).max(1_000_000).nullable(), reserved: z.number().int().min(0), revision: z.number().int().nonnegative().optional(), fileId: idSchema.optional(), calLink: z.string().max(2048).optional(), eventTypeId: z.number().int().positive().optional(), shipping: z.array(z.object({ country: z.string().regex(/^[A-Z]{2}$/), amount: moneySchema }).strict()).max(250), createdAt: z.string().datetime({ offset: true }) }).strict();
