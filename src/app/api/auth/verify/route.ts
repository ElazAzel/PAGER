...
export async function POST(request: Request) { return route(request, async () => {
  ...
  const input = z.object({ email: emailSchema, token: z.string().regex(/^\d{6,10}$/), role: z.enum(["creator", "buyer"]).optional() }).strict().parse(await body(request));
  ...
  const user = await currentUser(input.role); if (!user) throw new ApiError(401, "Verified session unavailable"); return json({ user });
}, true); }
