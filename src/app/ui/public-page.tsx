...
export function AuthModal({ locale, demoEnabled = false, onClose, onComplete, restoreFocusRef }: { locale: Locale; demoEnabled?: boolean; onClose: () => void; onComplete: (user: User) => void; restoreFocusRef?: FocusReturnRef }) {
...
export function ItemDetailScreen({ slug, itemId, initialPage, initialItem, initialBlockId = "" }: { slug: string; itemId: string; initialPage?: PublicPage; initialItem?: CatalogItem; initialBlockId?: string }) {
  ...
  const purchase = async () => { track("click", blockId); if (!user || user.role !== "buyer") { const returnTo = `/${slug}/items/${itemId}?blockId=${encodeURIComponent(blockId)}`; router.push(`/login?returnTo=${encodeURIComponent(returnTo)}`); return; } if (item.kind === "physical") { setShippingOpen(true); return; } await submitPurchase(); };
