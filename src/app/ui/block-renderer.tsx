...
export function BlockRenderer({ block, items = [], locale = "ru", slug, editor = false, library = false, onSelect, onDuplicate, onToggleHidden, onDelete, onBuyBlock, onItemBuy, onItemDetails, onBook, onLead }: Props) {
  ...
  return <article className={`public-block ${block.type} ${block.width} ${block.hidden ? "library-hidden" : ""}`}>{content}{library && block.hidden && <div className="library-label"><Icon name="Eye" size={12} />{locale === "ru" ? "Скрыт на странице · доступ сохранён" : "Hidden on page · access retained"}</div>}</article>;
}
