...
type Props = {
  ...
  onBuyBlock?: (block: RenderBlock) => void;
...
function ItemCard({ item, block, locale, slug, onBuy, onDetails, onBook }: { item: CatalogItem; block: RenderBlock; locale: "ru" | "en"; slug?: string; onBuy?: Props["onItemBuy"]; onDetails?: Props["onItemDetails"]; onBook?: Props["onBook"] }) {
  ...
  return <article className="item-card">
    ...
    <div className="item-actions">
      ...
      {service ? <button className="button button-primary" disabled={!onBook} onClick={() => onBook?.(item, block)}>{locale === "ru" ? "Записаться" : "Book"}</button> : <button className="button button-primary" disabled={unavailable} onClick={() => onBuy?.(item, block)}>{unavailable ? (locale === "ru" ? "Нет" : "Sold out") : (locale === "ru" ? "Купить" : "Buy")}</button>}
    </div>
  </article>;
}
...
function Faq({ data }: { data: BlockData }) {
  ...
  return <div className="faq-list">{(data.items ?? []).map((item, index) => <div className="faq-item" key={`${item.title}-${index}`}><button className="faq-question" onClick={() => setOpen(open === index ? null : index)}><span>{item.title}</span><Icon name={open === index ? "ChevronDown" : "ChevronRight"} size={15} /></button>{open === index && <div className="faq-answer">{item.text}</div>}</div>)}</div>;
}
...
function Gallery({ data, beforeAfter = false }: { data: BlockData; beforeAfter?: boolean }) {
  ...
  const images = (data.items ?? []).map(item => item.image).filter(Boolean) as string[];
  if (beforeAfter) return <div className="visual-block before-after"><div className="before-after-media"><img src={data.beforeImage || data.afterImage || ""} alt="" /><div className="after-layer" style={{ width: `${position}%` }}><img src={data.afterImage || data.beforeImage || ""} alt="" /></div></div><label className="sr-only" htmlFor={`before-after-${position}`}>Compare images</label><input id={`before-after-${position}`} type="range" min="0" max="100" value={position} onChange={event => setPosition(Number(event.target.value))} /></div>;
  if (!images.length) return <div className="media-placeholder"><Icon name="Image" size={25} /></div>;
  return <div className="visual-block"><img src={images[index]} alt="" /><div className="gallery-controls"><button className="icon-button" onClick={() => setIndex((index - 1 + images.length) % images.length)} aria-label="Previous image"><Icon name="ChevronLeft" /></button><span>{index + 1} / {images.length}</span><button className="icon-button" onClick={() => setIndex((index + 1) % images.length)} aria-label="Next image"><Icon name="ChevronRight" /></button></div></div>;
}

export function BlockRenderer({ block, items = [], locale = "ru", slug, editor = false, library = false, onSelect, onDuplicate, onToggleHidden, onDelete, onBuyBlock, onItemBuy, onItemDetails, onBook, onLead }: Props) {
  ...
  if (locked) return <article className={`public-block locked-block ${block.width}`}><div className="lock-content"><div className="lock-icon"><Icon name="LockKeyhole" size={18} /></div><h3>{data.title || meta[locale]}</h3><p>{block.teaser || (locale === "ru" ? "Материал открывается после покупки." : "Available after purchase.")}</p><div className="block-cta"><span className="item-price">{block.pricing.oneTime ? formatMoney(block.pricing.oneTime, block.pricing.currency, locale) : block.pricing.monthly ? `${formatMoney(block.pricing.monthly, block.pricing.currency, locale)} / ${locale === "ru" ? "мес." : "mo"}` : ""}</span><button className="button button-primary" onClick={() => onBuyBlock?.(block)}>{locale === "ru" ? "Открыть" : "Unlock"}</button></div></div></article>;

  const content = (() => {
    switch (block.type) {
      case "profile": return <div className="public-profile-block"><div className="avatar avatar-lg">{data.avatar ? <img src={data.avatar} alt="" /> : initials(data.name)}</div><div><h2>{data.name || data.title}</h2><div className="public-role">{data.profession || data.subtitle}</div><p className="rich-text" dangerouslySetInnerHTML={{ __html: safeHtml(data.text || "") }} /></div></div>;
      ...
      case "image": return <div className="visual-block">{data.image ? <img src={data.image} alt={data.title || ""} /> : <div className="media-placeholder"><Icon name="Image" size={25} /></div>}{data.title && <div className="public-block-content"><h3>{data.title}</h3></div>}</div>;
      ...
      case "video": return <><div className="video-placeholder"><Icon name="Play" size={30} /></div><div className="public-block-content"><h3>{data.title || meta[locale]}</h3><p>{data.text}</p></div></>;
      ...
      case "map": return <a className="public-link" href={safeHref(data.url) || "#"}><div className="public-link-icon"><Icon name="MapPin" size={17} /></div><div className="public-link-copy"><strong>{data.title || meta[locale]}</strong><span>{data.address || (locale === "ru" ? "Добавьте адрес" : "Add an address")}</span></div><Icon name="ArrowRight" size={16} /></a>;
      ...
      case "shoutout": { const href = safeHref(data.url); return <a className="public-link" href={href || "#"}><div className="public-link-icon"><Icon name="HeartHandshake" size={17} /></div><div className="public-link-copy"><strong>{data.title || data.name}</strong><span>{data.text}</span></div><Icon name="ArrowRight" size={16} /></a>; }
      ...
      case "custom_code": return <iframe className="code-frame" title={data.title || "Custom widget"} sandbox="allow-scripts" srcDoc={safeHtml(data.html || "")} />;
      ...
      case "booking": { const item = items.find(candidate => data.itemIds?.includes(candidate.id) && candidate.kind === "service"); return <div className="public-block-content"><h3>{data.title || meta[locale]}</h3><p>{data.text}</p><div className="block-cta"><span className="muted small"><Icon name="CalendarClock" size={14} /> {locale === "ru" ? "60 минут" : "60 minutes"}</span><button className="button button-primary" onClick={() => onBook?.(item, block)}>{data.label || (locale === "ru" ? "Записаться" : "Book")}</button></div></div>; }
  ...
  if (editor) {
    ...
    return <div className={`editor-block ${block.width} ${block.hidden ? "is-hidden" : ""}`} onClick={() => onSelect?.(editorBlock)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelect?.(editorBlock); } }} role="button" tabIndex={0} aria-label={`${labelFor(block.type, locale)}: ${plain(data.title || data.name || "")}`}>
  ...
  return <article className={`public-block ${block.width} ${block.hidden ? "library-hidden" : ""}`}>{content}{library && block.hidden && <div className="library-label"><Icon name="Eye" size={12} />{locale === "ru" ? "Скрыт на странице · доступ сохранён" : "Hidden on page · access retained"}</div>}</article>;
}
