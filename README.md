...
**В разработке.** Реализована серверная основа MVP и локальные демонстрационные сценарии. Мобильный интерфейс ещё не реализован: новые макеты находятся в `docs/design/mobile`, пользователь выбирает направление. Наличие API и проходящих тестов не означает готовность полного продукта.
